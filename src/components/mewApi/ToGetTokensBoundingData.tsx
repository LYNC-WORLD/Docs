...
const BODY = {
  fields: [
    ...
    },
    {
      name: "asset_id",
      type: "string",
      description: "The unique identifier of the asset you want to retrieve.",
...
const GET_SAMPLE_CODE = (sampleCodeParams: SampleCodeParams) => {
  const { network, endPoint, projectApiKey, xApiKey, asset_id } =
    sampleCodeParams;

  // build query params string to match API call
  const queryParams = `?asset_id=${asset_id}&network=${network}&apiKey=${projectApiKey}`;
...
export const ToGetTokensBoundingData = () => {
  ...
  const [asset_id, setasset_id] = useState<string>(
  ...
  }, [dashboardApiKey, xApiKey, network, asset_id]);
            ...
            <div>
              {BODY.fields
                ...
                .map((field, index) => {
                  ...
                  const isNetwork = field.enum;
                  const isasset_id = field.name === "asset_id";
                      ...
                      <div className="flex items-center justify-between px-5 py-3 text-gray-900 whitespace-nowrap dark:text-white">
                        ...
                        {isNetwork ? (
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              isApiKey
                                ? setDashboardApiKey(e.target.value)
                                : isasset_id
