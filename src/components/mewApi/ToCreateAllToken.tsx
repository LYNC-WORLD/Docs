...
export const ToCreateAllToken = () => {
  ...
  }, [dashboardApiKey, xApiKey, network, limit, page, sortBy, order]);

  const handleApiCall = async () => {
    try {
      setFetchingResponse(true);
      // fixed query string concatenation (added & between params)
      const queryParams = `?network=${network}&page=${page}&limit=${limit}&sortBy=${sortBy}&order=${order}&apiKey=${dashboardApiKey}`;
        ...
        <div>
          <div className="lg:pr-5 w-full">
            ...
            <div>
              ...
              {/* Nested fields: page, limit, sortBy, order */}
                    ...
                    <div className="flex items-center justify-between px-5 py-3 text-gray-900 whitespace-nowrap dark:text-white">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          else if (key === "order") setOrder(val);
                        }}
                        className="w-1/2 md:w-48 py-[0.6rem] px-[0.8rem] outline-none rounded-[var(--ifm-global-radius)] resize-none border-[length:var(--ifm-global-border-width)] border-[var(--ifm-toc-border-color)] border-solid"
                      />
                    </div>
