...
const GET_SAMPLE_CODE = (sampleCodeParams: SampleCodeParams) => {
  ...
  const queryParams = `?network=${network}&page=${page}&limit=${limit}&sortBy=${sortBy}&search=${search}`;
...
export const ToSearchForToken = () => {
  ...
  const [search, setSearch] = useState<string>("Mew");
  ...
  }, [dashboardApiKey, xApiKey, network, limit, page, sortBy, search]);
