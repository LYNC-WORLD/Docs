  ...
  fields: [
    ...
    },
    {
      ...
      userId: {
        name: "userId",
        type: "string",
        description: "userId term to filter results",
        required: true,
        example: "0x38ac030b0fcdf4069ee5ba54ca6ce71c3c59c135",
      },
    },
  ],
};
...
const GET_SAMPLE_CODE = (sampleCodeParams: SampleCodeParams) => {
  const { network, endPoint, projectApiKey, xApiKey, page, limit, userId } =
    sampleCodeParams;

  // build query params string to match API call
  const queryParams = `?network=${network}&page=${page}&limit=${limit}&userId=${userId}&apiKey=${projectApiKey}`;
...
export const ToGetTokensHeldByUser = () => {
  ...
  const [userId, setuserId] = useState<string>(
    "0x38ac030b0fcdf4069ee5ba54ca6ce71c3c59c135"
  );
  ...
  const [sampleCode, setSampleCode] = useState<string>(
    GET_SAMPLE_CODE({
      ...
      page: "1",
      limit: "10",
      userId: "0x38ac030b0fcdf4069ee5ba54ca6ce71c3c59c135",
    })
  );
  ...
  }, [dashboardApiKey, xApiKey, network, limit, page, userId]);

  const handleApiCall = async () => {
    try {
      setFetchingResponse(true);
      const queryParams = `?network=${network}&page=${page}&limit=${limit}&userId=${userId}&apiKey=${dashboardApiKey}`;
                    ...
                    <div className="flex items-center justify-between px-5 py-3 text-gray-900 whitespace-nowrap dark:text-white">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          else if (key === "userId") setuserId(val);
                        }}
                        className="w-1/2 md:w-48 py-[0.6rem] px-[0.8rem] outline-none rounded-[var(--ifm-global-radius)] resize-none border-[length:var(--ifm-global-border-width)] border-[var(--ifm-toc-border-color)] border-solid"
                      />
                    </div>
