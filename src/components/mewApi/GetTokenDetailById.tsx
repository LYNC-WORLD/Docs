...
const GET_SAMPLE_CODE = (sampleCodeParams: SampleCodeParams) => {
  const { network, endPoint, projectApiKey, xApiKey, assetId } =
    sampleCodeParams;

  // build query params string to match API call
  const queryParams = `${assetId}?network=${network}&apiKey=${projectApiKey}`;
...
export const GetTokenDetailById = () => {
  ...
  }, [dashboardApiKey, xApiKey, network, assetId]);
            ...
            <div>
              {BODY.fields
                ...
                .map((field, index) => {
                  ...
                  const isAssetId = field.name === "assetId";
