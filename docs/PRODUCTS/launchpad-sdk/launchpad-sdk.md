---
sidebar_label: "Launchpad SDK"
id: launchpad-sdk
custom_edit_url: null
---

# Launchpad SDK

Introducing Launchpad NPM SDK. Powered by LYNC, Launchpad NPM SDK allows developer to integrate token launchpad inside any dapp in a few lines of code. It simplifies complex operations such as token creation, AMM pool initialization, buy/sell interactions, and real-time price tracking, allowing developers to focus on building great user experiences.

## Demo Dapp

<div className="flex items-center flex-col mb-5">

    <a href="https://metamask-example.lync.world/" target="_blank" className="mb-5 w-full border-[0.5px] hover:border-[rgb(135,135,135)] cursor-pointer border-[rgb(91,91,91)] border-solid rounded-[.25rem] flex items-center justify-between px-5 py-2">
        <div className="flex items-center gap-4">
            <img className="w-10" src="/img/lync-logo.png" alt="" />
            <span className="mt-1">Launchpad SDK Integration Example | LYNC</span>
        </div>
        <div>
            <img className="w-3 mt-2" src="/img/arrow.svg" alt="" />
        </div>
    </a>
    <span className="text-sm text-[rgb(192,192,192)]">Example LaunchpadSDK Integration Dapp</span>

</div>

## Package link on [npmjs](https://www.npmjs.com/)

<div className="flex items-center flex-col mb-5">

    <a href="https://www.npmjs.com/package/lync-wallet-sdk" target="_blank" className="mb-5 w-full border-[0.5px] hover:border-[rgb(135,135,135)] cursor-pointer border-[rgb(91,91,91)] border-solid rounded-[.25rem] flex items-center justify-between px-5 py-2">
        <div className="flex items-center gap-4">
            <span className="mt-1">https://www.npmjs.com/package/lync-wallet-sdk</span>
        </div>

        <div>
            <img className="w-3 mt-2" src="/img/arrow.svg" alt="" />
        </div>
    </a>
    <span className="text-sm text-[rgb(192,192,192)]">Link to package on npmjs</span>

</div>

## View code on GitHub

<div className="flex items-center flex-col mb-5">

    <a href="https://github.com/LYNC-WORLD/LYNC-launchpad-SDK" target="_blank" className="mb-5 w-full border-[0.5px] hover:border-[rgb(135,135,135)] cursor-pointer border-[rgb(91,91,91)] border-solid rounded-[.25rem] flex items-center justify-between px-5 py-2">
        <div className="flex items-center gap-4">
            <span className="mt-1">https://github.com/LYNC-WORLD/LYNC-launchpad-SDK</span>
        </div>
        <div>
            <img className="w-3 mt-2" src="/img/arrow.svg" alt="" />
        </div>
    </a>
    <span className="text-sm text-[rgb(192,192,192)]">Link to package on github</span>

</div>

## View example application code on GitHub

<div className="flex items-center flex-col mb-5">

    <a href="https://github.com/LYNC-WORLD/Launchpad-SDK-Demo" target="_blank" className="mb-5 w-full border-[0.5px] hover:border-[rgb(135,135,135)] cursor-pointer border-[rgb(91,91,91)] border-solid rounded-[.25rem] flex items-center justify-between px-5 py-2">
        <div className="flex items-center gap-4">
            <span className="mt-1">https://github.com/LYNC-WORLD/Launchpad-SDK-Demo</span>
        </div>
        <div>
            <img className="w-3 mt-2" src="/img/arrow.svg" alt="" />
        </div>
    </a>

    <span className="text-sm text-[rgb(192,192,192)]">Link to example dapp code on GitHub</span>

</div>
