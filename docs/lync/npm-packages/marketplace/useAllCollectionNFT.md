...
```typescript
function useAllCollectionNFT(contractAddress?: string, chainId?: string);
```
...
function App() {
  const { allNfts, isLoading, contractMetadata } = useAllCollectionNFT(
